...
The agents share one methodology. Each knows where it fits in the workflow, when to hand off, and when to push back.
...
**Lead: Tara → Sato.** Strict TDD. Tara writes failing tests first, then Sato makes them pass. One commit per issue, conventional commit format. No shortcuts.
