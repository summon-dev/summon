---
agent-notes: { ctx: "ADR: multi-runtime install (claude/copilot/both), manifest, generation-as-projection, CLI verb set", deps: [docs/adrs/template.md, docs/adrs/meta/0004-summon-doctor.md, docs/adrs/meta/0005-behavioral-benchmark.md, packages/summon-team/src/index.ts, packages/summon-team/src/doctor.ts], state: accepted, last: "claude@2026-08-06", key: ["manifest schema = § Additional Decisions #6", "Copilot output is a projection of .claude/*, never hand-authored", "targets: claude (default) | copilot | both = Cases A/C/B"] }
---
...
**Amended by ADR-0015 (2026-09-11):** the Case-A clause of § Additional Decisions #1 ("a Claude-only install writes no `.summon/manifest.json`") is superseded; under the v3 team every scaffold writes the manifest, with two optional fields (`scaffolded`, `source`) added to #6's schema. The location, field names, `manifestVersion`, and forward-compatible reading stand.
...
1. **Default target is `claude`; Case A is byte-for-byte today's behavior.** Because Case A writes no `.summon/manifest.json` (see #6), "byte-for-byte" is literal, not approximate: the flag is additive, and an existing user who never passes `--target` gets no new file and no changed behavior. Migration risk on the existing install path is zero.
   ...
   **One exception exists as of 2026-08-06, and it is stated here rather than left implicit.** ADR-0014's opt-in add-on writes an `addons[]` entry, which **forces a manifest to exist on a Case A repo** — the first and so far only thing that does. The claim above survives *as scoped*, because it fires only when the user answers yes to a prompt that defaults to no: the default Case A path still writes nothing, and an existing user who never opts in is unaffected. What is no longer true without qualification is "Case A never produces a manifest." Recording it here is deliberate — ADR-0014 § 5 required the implementation to state this in ADR-0006's own terms rather than quietly widening the claim from the other side.

   Two schema consequences land on #6 when the manifest module is built (issue #8): `addons` is **optional and additive**, so `manifestVersion` stays **1**; and the validator **must not be strict on unknown top-level keys**, so an older `summon-team` reading a newer manifest ignores fields it does not know instead of hard-failing. Forward compatibility is the point of a persistent on-disk artifact, and strictness would make every future additive field a breaking change.
