...
3. Remaining personas and roles ported one at a time, each port a diff against its v2 file that classifies every sentence as role, persona, view, adapter, check, or delete. (Landed, same day: nine roles and nine personas, a `write:infra` capability, an accessibility lens; the composite reviewer retired in favour of the formation. By non-blank line, the nine v2 files totalled 776 lines and their v3 role plus persona files total 401, a 48% reduction; the line classifier in the wave-2 commit message puts most surviving v2 lines in the role column, because the port compresses the work rather than deleting it, and the outright deletions are harness frontmatter, routing tables, directives, one command reference, and vendor price lists. A simplicity review of the seam was started and did not complete; it is owed before ratification.)
4. `check-canon` learns the new tree: a party binding must resolve, an adapter must carry `fitted: true`, view text must not appear in composed output, an `agent-notes` dep must resolve to a file, the log validates. The composer and the log tool already enforce most of these at run time; check-canon makes them CI facts. (Landed, same day, as check #10; its first run on the real tree caught two skin epithets that repeated prompt prose, which is the invariant working.)
