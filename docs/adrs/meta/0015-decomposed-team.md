---
agent-notes: { ctx: "ADR: v3 team decomposed into role / persona / view layers plus a fitted harness adapter, composed by script; checks split deterministic from judged", deps: [docs/adrs/template.md, docs/adrs/meta/0012-executable-canon.md, docs/adrs/meta/0007-canon-meta-boundary.md, docs/adrs/meta/0006-multi-runtime-install.md, docs/methodology/team-layers.md, team/README.md, scripts/compose-team.mjs, docs/history/tracking/2026-09-09-adr-0015-decomposed-team-debate.md, docs/history/tracking/2026-09-09-adr-0015-decomposed-team-dispositions.md], state: draft, last: "claude@2026-09-09", key: ["four layers, three of them portable; the harness adapter is the only file allowed to be fitted", "checks: role claims a command can decide, bound per project; the rest is judgment", "supersedes the register stack (#95 #105 #116 #120 #131) by the human's direction", "Wei round 1: 11 challenges, 2 blocking, dispositions in the debate record; Archie's review has not run"] }
---
...
1. This ADR, the canon spec, the `team/` tree for the implementation trio and the review formation, the composer with tests, the `skills` and `claude-code` adapters, the checks bindings for this repo. (This wave; landed.)
2. Remaining personas and roles ported one at a time, each port a diff against its v2 file that classifies every sentence as role, persona, view, adapter, check, or delete. The delete column is expected to be the largest.
3. `check-canon` learns the new tree: a party binding must resolve, an adapter must carry `fitted: true`, view text must not appear in composed output, an `agent-notes` dep must resolve to a file. The composer already enforces the first three at compose time; check-canon makes them CI facts.
4. The check runner: executes bound checks, binds receipts to a tree state per ADR-0012's receipt schema, and computes the reviewer's `disagreement-rate` from review artefacts. The `negative-control` fixture (a planted-defect diff the formation must not wave through) lands with it.
5. Cutover: `.claude/agents/` becomes composer output, gitignored or generated in CI, and `personas.md` becomes a rendered roster rather than a source. Requires ratification and a fidelity read of the composed files against the v2 originals. `check-canon`'s agent-file checks are taught the composed shape first.
6. The scaffolder ships `team/` and the composer, runs compose at install, and `doctor` runs `compose --check`. A downstream project gets the layers, not fused output.
7. Site `TeamGrid.astro` reads the view's `party.json` plus the party binding instead of its own hard-coded table.

### Reversal triggers (90-day checkpoint: 2026-12-08)

Per Wei's C8, adopted verbatim. At the checkpoint, or earlier if tripped:

1. **Consensus returns.** The composed review formation fails the negative-control fixture, or returns unanimous verdicts on ten consecutive real reviews. The persona layer is not producing dissent; re-argue Alternative 2 (skills only) on the evidence.
