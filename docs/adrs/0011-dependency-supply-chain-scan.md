...
- **G. Recommend-and-scaffold, default-on and removable (the team's original recommendation).** Rejected at ratification in favour of the opt-in posture (D). It would ship a working control into every scaffolded project by default — symmetric with ADR-0010's default-on cooldown — but the owner judged the free tool's net-new signal over the existing `audit` step too modest to justify a default-on footprint, and preferred that nothing scanner-shaped land in a project that never asked for it. Recorded so the rejected majority recommendation, and the reason it lost, are on file.
