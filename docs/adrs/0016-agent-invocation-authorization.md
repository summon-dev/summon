...
The distinction is load-bearing, not stylistic. An earlier draft said *"where a session restriction contradicts this, state the conflict in one line and proceed."* That framing concedes a conflict which, on this ADR's own logic, does not exist — and a coordinator that believes it is overriding the runtime is one bad inference from believing it may. Reading the condition as satisfied has an intrinsic stopping point; overriding has none.
