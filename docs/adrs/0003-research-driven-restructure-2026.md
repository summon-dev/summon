...
| **ADR-D** | Persona split: agents vs skills, with migration list | MUST justify each migration persona-by-persona, citing accountability, tool isolation, and depth-of-expertise consequences |
...
| **ADR-G** | Slim rarely-active personas to summoned specialists | MUST cite the anti-pattern each migration avoids (e.g., Invisible UI for Dani per `docs/process/gotchas.md`); MAY conclude that a given persona is *not* a candidate for migration |
