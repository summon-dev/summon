...
**Carrying this line is what puts an agent under the contract.** The delivery mechanism is the scope: an agent whose file carries the line owes a PACKET, and an agent type with no `.claude/agents/` file — a built-in the harness provides, such as `general-purpose` — never received it and owes nothing. That follows from #112 rather than adding to it, and anything measuring or enforcing compliance has to use the same predicate. Two cautions, both learned by getting them wrong: a file that exists but carries no binding line is **not** bound, and an agent absent from a roster that was read is a **broken roster**, not an excused agent. Absence of evidence that an agent was bound is not evidence that it was not.
