...
Every project carries `team/version.json`: the summon-team version its team came from, when it was scaffolded, and from where (`{ "summon-team": "0.1.0", "scaffolded": "<ISO timestamp>", "source": "github:<template>" }`; in this repo the last two are null). The scaffolder writes it at scaffold time from its own build-baked version, overwriting whatever the template carried, so the file is a fact about the project and not a copy of the source's. This repo's own copy is kept equal to `packages/summon-team/package.json` by a canon check. `team-log.mjs --version` prints the `summon-team` field, bare, as the scriptable form; `summon-team --version` prints the human one. The point is upgrades: a later pass, by a person or a model, reads the manifest to learn which version of the layers a project is on, and patches the difference rather than guessing from the files.
